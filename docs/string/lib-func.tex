...
\texttt{sscanf(const char *__source, const char *__format, ...)}：从字符串\texttt{__source}里读取变量，比如\texttt{sscanf(str,"%d",&a)}。

\subsubsection{sprintf}

\texttt{sprintf(char *__stream, const char *__format, ...)}：将\texttt{__format}字符串里的内容输出到\texttt{__stream}中，比如\texttt{sprintf(str,"%d",i)}。
...
\texttt{char *strcpy(char *str, const char *src)} : 把 \texttt{src} 中的字符复制到 \texttt{str} 中， \texttt{str} \texttt{src} 均为字符数组头指针， 返回值为 \texttt{str} 包含空终止符号 \texttt{'\0'} 。
...
\texttt{char *strcat(char *str1, const char *str2)} : 将 \texttt{str2} 接到 \texttt{str1} 的结尾，用 \texttt{*str2} 替换 \texttt{str1} 末尾的 \texttt{'\0'}  返回 \texttt{str1} 。
...
\item 访问函数 \texttt{data() / c_str()} 返回一个\texttt{const char*} 指针, 内容与该 \texttt{string} 相同。
